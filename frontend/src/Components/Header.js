        ...
        <>
            <Navbar bg="light" variant="light" className='px-4 justify-content-center'>
                ...
                <div onClick={() => showMenu()} className='mobile-nav'>
                    <h3 className='task-title'>{(boardEdit && board.length > 0 && boardEdit.id === board[0].board.id) 
                        ? boardEdit.name : (board && board.length > 0) 
                        ? board[0].board.name : ('Select a board')}</h3>
                ...
                <div id="mobile-menu" className={showMobileMenu ? 'show' : ''}>
                    <div className='task-modal' onClick={hideModal}>
                        <Sidebar screen={'mobile'}/>
                    </div>
                </div>

                <div className='ms-auto relative d-flex align-items-center'>
                    ...
                    <button onClick={() => navigate(`/board/${id}/new-task`)}
                        disabled={btnDisabled} id='mobile-add-task' className={`me-2 add-task ${btnDisabled ? ' opacity-25' : ''} btn-primary-l`}>
                    ...
                    <div className={`modal-actions ${showActions ? 'd-block' : 'd-none'}`}>
                        ...
                        <p onClick={() => handleDelete()} className="delete-task">Delete Board</p>
                    </div>
                </div>
